...
const isAuthenticated = async (req, res, next) => {
    try {
        ...
        if (!token) {
            return res.status(401).json(new ApiError(401, "Authentication failed. Please login."));
        }
        ...
        if (!user) {
            return res.status(404).json(new ApiError(404, "Authentication failed. User not found."));
        }
